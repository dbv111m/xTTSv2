...
**Для тестирования интерфейса без зависимостей:**
```bash
# Откройте статическую версию в браузере
test_static.html
```
...
1. Выберите вкладку "Text to Speech"
2. Введите текст
3. Выберите язык и голос (опционально)
4. Нажмите "Choose Reference Audio" и загрузите файл
...
- **Два формата вывода**: WAV и MP3 (по умолчанию MP3)
- **Умное именование файлов**: `YYYY-MM-DD_HH-MM_(voice_name).format` (пробелы заменяются на _)
...
# Показывать вкладку API Info
SHOW_API_INFO_TAB=false
```
...
# Файл сохранится с именем типа "2025-01-15_14-30_Daisy_Studios.mp3"
...
├── test_app.py       # Тестовый сервер без зависимостей
├── test_static.html  # Статическая тестовая версия интерфейса
